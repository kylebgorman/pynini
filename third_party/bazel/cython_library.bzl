...
"""Custom rules from gRPC Python with local modifications."""
...
def pyx_library(name, deps = [], py_deps = [], srcs = [], **kwargs):
    ...
    for filename in pyx_srcs:
        native.genrule(
            ...
            cmd =
                "PYTHONHASHSEED=0 $(location @org_cython//:cython_binary) --cplus $(SRCS) --output-file $(OUTS)",
            tools = ["@org_cython//:cython_binary"] + pxd_srcs,
        )
